...
- For each request, a new MetaMask SDK instance is created, similar to how OAuth generates a unique authorization flow for each session.
...
- The QRcode will be generated by the universalLink composed of MetaMask connect base URL, channelId and pubKey.

### Scan the QRcode
Now(25/09/2024) the chatGPT client can not synchronize the status like MetaMask SDK's in the actions server. So you must explicit instruction to chatGPT that you have sanned the QRcode.
