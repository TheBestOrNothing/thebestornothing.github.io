...
```
eth_signTypedData_v4 presents a data message for the user to sign in a structured and readable format and returns the signed response. At last, the **MetaMask token(mmToken)** is composed of expiration time, account, and signature. 
...
the ChatGPT action API currently does not support custom headers, including the Authorization header. This limitation means that you cannot send the access token in the header as per the standard OAuth 2.0 practice. As a result, you have to include the token in the POST body instead.
