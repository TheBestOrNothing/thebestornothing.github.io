...
3. Prompt for QR Code Scanning: ask the user: "Have you scanned the MetaMask QR code to connect your account?" If the user responds Yes, proceed by exchanging the MetaMask authorization code for a MetaMask token.
4. Retrieve MetaMask Token: Receiving the MetaMask token via the authorization server.
...
sequenceDiagram
    ...
    chatGPTClient->>MetaMask-GPTIdentityPlatform: Request access_token with authorization code
    MetaMask-GPTIdentityPlatform->>MetaMaskMobile: sign of access_token  request
    MetaMaskMobile->>MetaMask-GPTIdentityPlatform: return sign of access_token 
    MetaMask-GPTIdentityPlatform->>chatGPTClient: sign, account, expired time composed as access_token
